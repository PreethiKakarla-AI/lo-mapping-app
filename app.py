...
with st.container():
    ...
    bloom_desc = refs["bloomlevel"].iloc[:, -1].dropna().unique().tolist()
    bloom_level = st.selectbox("Bloom Level", bloom_desc)
    activity = st.selectbox("Activity", refs["activity"].iloc[:, -1].dropna().unique().tolist())
